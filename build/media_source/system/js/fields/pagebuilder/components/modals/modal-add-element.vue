		...
		<div class="modal-body">
			<div class="row">
				...
				<div class="col-8 tab-content">
					<div v-for="element in allowedChildren" :id="'list-'+element.id" class="tab-pane fade"
						 role="tabpanel">
						{{ element.description }}
						...
						<div v-for="(conf, id) in element.config" v-if="conf.required" class="element-configuration">
							...
							<input v-else :type="conf.type" :id="id" :name="id" class="form-control" required
								   :placeholder="conf.placeholder" v-model="config[id]"
								   @blur="check(element.id, $event)"/>
						</div>
					</div>
				</div>
			</div>
			<small class="form-text text-muted add_element_desc">{{ translate('JLIB_PAGEBUILDER_ADD_ELEMENT_DESC')
				}}</small>
		</div>
    ...
    export default {
        ...
        methods: {
            add() {
				...
				// console.log(this.selectedElement, this.config, this.childConfig);
